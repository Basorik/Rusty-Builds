...
| `update_selected_nodes` | `node_ids: Vec<u32>` | `Result<BuildStats, String>` | ⚠️ Partial — STR/DEX/INT from `granted_*` fields; other stats via `StatAccumulator` placeholder |
| `get_available_tree_versions` | `app: AppHandle` | `Result<Vec<String>, String>` | ✅ Working — scans `data/tree/` directories |
| `load_tree_version` | `version: String` | `Result<(), String>` | ✅ Working — validates input, replaces `GameData` in `RwLock` |
| `get_tree_json` | `app: AppHandle` | `Result<String, String>` | ✅ Working — serves raw JSON string for frontend |
...
| `GameData` | `Arc<RwLock<GameData>>` | Managed Tauri state — holds `PassiveTree` + source names. Loaded from `data/tree/<version>/data.json` at startup. Mutable via `load_tree_version` |
...
├── lib.rs                      # App setup, commands, types (BuildInfo, BuildStats, Class, etc.), DEFAULT_TREE_VERSION
├── stats.rs                    # StatAccumulator — placeholder stat parser (Phase 2 replaces with ModDB)
├── models.rs                   # LiteNode (rkyv-serialized, legacy — unused)
├── commands.rs                 # Empty placeholder for future command extraction
├── tree.rs                     # Empty placeholder
├── data/
│   ├── mod.rs                  # DataLoader trait, DataError, GameData struct, SourceId newtype
│   ├── stat_id.rs              # StatId enum (320 variants from SkillStatMap.json)
│   ├── tree.rs                 # PassiveTree, PassiveNode, PassiveGroup, ClassData, etc. (fully working + tests)
│   ├── bases.rs                # Empty stub
│   ├── gems.rs                 # Empty stub
│   ├── mods.rs                 # Empty stub
...
├── data.json                   # PoE 1 passive tree data (~160k lines) — UNUSED, frontend fetches from Rust via get_tree_json
├── routes/
│   ├── +layout.ts              # ssr = false
...
pub struct BuildStats {
    pub total_strength: i32,     // Summed from PassiveNode.granted_strength
    pub total_dexterity: i32,    // Summed from PassiveNode.granted_dexterity
    pub total_intelligence: i32, // Summed from PassiveNode.granted_intelligence
    pub node_count: u32,
...
### `StatAccumulator` (`stats.rs`)

Placeholder stat system. Parses numbers from display strings using regex, replaces numbers with `#` to form template keys, and sums values. Will be replaced by `ModDB` + `StatId` in Phase 2.
...
- **Tree data**: Fetches latest release tag from `grindinggear/skilltree-export` GitHub API, downloads `data.json` to `src-tauri/data/tree/{tag}/data.json`. Prints a reminder to update `DEFAULT_TREE_VERSION` in `lib.rs`
...
| `tree/<version>/data.json` | PoE 1 passive skill tree (nodes, groups, connections, constants) | `GameData.tree` (Rust), `SkillTree.svelte` (via `get_tree_json`) |
...
|---|---|---|---|
| **1** | Foundation & Data Layer | ✅ Complete | Load all game data into typed Rust structs |
| **2** | Modifier System & Passive Tree Stats | ❌ Not Started | `ModDB`, `parse_display_text`, typed stat accumulation |
...
- ✅ `data/mod.rs` — `DataLoader` trait, `DataError`, `GameData` struct, `SourceId` newtype
- ✅ `data/tree.rs` — `PassiveTree`, `PassiveNode` with `granted_*` fields, `ClassData` with `base_str/base_dex/base_int`, `get_node()` helper, unit tests
- ✅ `GameData` loaded from versioned `data/tree/<version>/data.json` at startup, managed as `Arc<RwLock<GameData>>`
- ✅ `DEFAULT_TREE_VERSION` constant in `lib.rs` (no symlinks)
- ✅ `get_tree_json` command serves raw JSON string to frontend (no Serialize on tree structs)
- ✅ Frontend fetches tree via `commands.getTreeJson()` on mount
- ✅ `StatAccumulator` in `stats.rs` sums stat values from display strings (placeholder)
- ✅ STR/DEX/INT computed from `PassiveNode.granted_strength/dexterity/intelligence`
- ✅ `load_tree_version` command with input validation (path traversal protection)
- ✅ `get_available_tree_versions` scans `data/tree/` for version directories
- ⬜ Load gems, bases, uniques, mods into typed structs (Phase 3-4)
- ⬜ Replace `StatAccumulator` with `StatId`-keyed `ModDB` (Phase 2)
...
- `data.json` in `src/` is unused — frontend fetches tree data from Rust via `get_tree_json` command
...
- **SkillStatMap**: Maps internal game stat IDs → calc engine variable names + types (BASE/INC/MORE/FLAG). This drives `StatId` and the future `parse_display_text` parser. In Phase 3+, also used for direct stat-to-modifier mapping for gems/skills (bypassing text parsing).
...
|---|---|---|
| `modifier/mod_db.rs` | `src/Classes/ModDB.lua` + `src/Classes/ModStore.lua` | 400+500 |
| `modifier/parser.rs` (`parse_display_text`) | `src/Modules/ModParser.lua` | 2500 |
...
The core calc formula: `Override check → base × (1 + sum_of_increases%) × product_of_mores`

PoB's ModDB has 8 mod types: `BASE`, `INC`, `MORE`, `FLAG`, `OVERRIDE`, `LIST`, `MAX`, `MIN`. Key query methods: `Sum()`, `More()`, `Flag()`, `Override()`, `List()`, `Tabulate()`, `Max()`, `Min()`. All accept a `cfg` (our `CalcContext`) parameter.
